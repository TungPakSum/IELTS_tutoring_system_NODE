...
router.get("/get/:uid", auth, async function (req, res, next) {
  try {
    ...
    const result = await db.collection("chats").find({ uid }).toArray();
...
try{
    let result = await db.collection("chats").deleteMany({ uid });
...
router.post("/save", auth, async function (req, res) {
  ...
  try {
    const result = await db.collection("chats").insertOne(message);
